...
import type { LeaderboardEntry, OMTrendData, LeaderboardPageData } from '@/types';
...
import { Users, TrendingUp, BarChart3, Crown, UserCheck, UserCog, AlertCircle } from 'lucide-react';
...
const getRagColor = (status: 'green' | 'amber' | 'red') => {
    switch (status) {
        case 'green': return 'hsl(var(--chart-2))'; // Use theme colors
                ...
                <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={data} layout="vertical" margin={{ top: 5, right: 10, left: 5, bottom: 5 }}>
                        ...
                        <Tooltip
                           ...
                           formatter={(value: number, name: string, props) => [`${value}%`, `RAG Score`]} // Format tooltip content
                        />
...
const OMTrendChart: React.FC<OMTrendChartProps> = ({ data }) => (
    <Card className="shadow-md border rounded-lg overflow-hidden">
        <CardHeader>
            ...
            {/* <CardDescription>
                Latest Ranks: TL - {data.subordinateRanks?.[data.subordinateRanks.length - 1]?.tlRank ?? 'N/A'}, SPM - {data.subordinateRanks?.[data.subordinateRanks.length - 1]?.spmRank ?? 'N/A'}
            </CardDescription> */}
        </CardHeader>
...
const LoadingSkeleton = () => (
    <div className="space-y-8">
        ...
        {/* Podium Charts Skeleton */}
         <h2 className="text-2xl font-semibold mb-4"><Skeleton className="h-6 w-40" /></h2>
        ...
        <h2 className="text-2xl font-semibold mb-4 mt-8"><Skeleton className="h-6 w-48" /></h2>
...
const LeaderboardPage: React.FC = () => {
    ...
    const { topPerformers, omTrends } = pageData;
